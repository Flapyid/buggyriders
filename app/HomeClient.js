      ...
      <section className="relative w-full flex flex-col md:flex-row items-center justify-between px-4 sm:px-6 md:px-8 lg:px-16 py-12 md:py-16 bg-gray-50">
        <div className="md:w-1/2 w-full flex justify-center md:justify-start mb-8 md:mb-0">
          <div className="relative w-full max-w-md lg:max-w-lg xl:max-w-xl">
            <Image src={sectionimg} alt="Polaris Buggy in desert adventure" width={600} height={800} placeholder="blur" className="rounded-lg shadow-md" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
          </div>
        </div>
        <div className="md:w-1/2 w-full md:pl-8 lg:pl-12 xl:pl-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-orange-600 mb-4 md:mb-6">EXTREME DESERT TOURS</h2>
          <div className="space-y-3 md:space-y-4 text-gray-800">
            <p className="text-base sm:text-lg leading-relaxed">We at Buggy Riders have put together a selection of thrilling desert activities to satisfy the desires of every thrill-seeker...</p>
            <p className="text-base sm:text-lg leading-relaxed">Dubai's desert offers a variety of activities to enjoy against the stunning backdrop of endless dunes and starry nights...</p>
          </div>
          <button className="mt-6 md:mt-8 px-6 py-2 md:px-8 md:py-3 bg-orange-600 text-white font-semibold rounded-full hover:bg-orange-700 transition-colors duration-300 text-base md:text-lg">Explore Our Tours</button>
        </div>
      </section>
    )}

    <QuickEnquiryModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} serviceName={title} />
  </>);
}
